...
SXCS with \emph{best selection} showed serious problems in the \emph{difficult scenario} while it was able manage the problem with a more random \emph{tournament selection}. This points to a correctable flaw in the design of the algorithm (probably with the \emph{neutral events}). But other than XCS SXCS \emph{can} handle advanced base reward functions, e.g. with a collaborative element (evading other agents) which significantly helps for example in the \emph{difficult scenario} though this is probably due to the fact that it simply encourages agents to move away from other agents, explore the grid and move through the openings. But in the end SXCS was designed to follow and observe a moving goal object, the \emph{difficult scenario} is more like a labyrinth where finding the goal object in the first place is important.

Besides improving the SXCS algorithm itself future research could be pointed at a switching strategy between explore (when no goal object is in sight) and exploit phases (when it is near the goal object) and improvement can be expected from an adaptive \emph{learning rate} and \emph{maxStackSize} to fit the scenario's needs.
