...
A promising modified XCS approach has been investigated to overcome the drawbacks of the classical XCS algorithm in the field of \emph{non observable Markov decision processes}. The proposed idea is mainly based on expanding the local reward function and some kind of temporary memory, which stores past actions sets and their corresponding reward values. Thus, local payoffs can be delayed to be analyzed later and the reward function reflects in a better way the local agent behavior. In the experiments this new approach proved to be superior to the classical XCS algorithm, mainly due to its ability to work well with advanced local reward functions, an ability which XCS seemingly lacks.Besides improving the new algorithm itself, future research could be pointed at a switching strategy between explore (when no prey is in sight) and exploit phases (when it is near the prey). In addition, improvement can be expected from an adaptive \emph{learning rate} $\beta$ and \emph{maxStackSize} to fit the current scenario's needs. Finally, it would be interesting to test the approach of \emph{eventXCS} in connection with standard scenarios and with other \emph{non observable Markov decision processes} already discussed in literature.
